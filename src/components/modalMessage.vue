...
export default {
  ...
  methods: {
    ...
    getMessage() {
      ...
      this.$http
        .get("/omsIm/demo/json/getList.php", {
          params: {
        ...
        .then(response => {
          ...
          if (response.data) {
            this.p++;
            this.messagesLog.unshift.apply(
              this.messagesLog,
              response.data.data.reverse()
            );
            vm.$nextTick(function() {
              _this.$refs.scrollBox.scrollTop =
                _this.$refs.scroll.offsetHeight - scrollHeight;
    ...
    forward(content) {
      ...
      this.shareType = this.forwardInfo.mesages_types;
...
<template>
  <div>
    ...
    <asyncModal v-if="show" @close="close">
      <div slot="header">与 {{ dig.name }} 的聊天记录</div>
          ...
          <li v-for="item in messagesLog">
            ...
            <div class="main" :class="{ self: item.id == user.id }">
              <img
                ...
                width="40"
                height="40"
                :src="item.id == user.id ? user.documentUrlOms  + user.img : (item.avatar.indexOf('http') == -1 ? user.documentUrlOms  + item.avatar : item.avatar )"
              />

              <div
                v-if="currentSessionType == 'groupMessage' && item.id != user.id"
                class="groupName"
              >{{item.username}}</div>
              ...
              <div
                ...
                class="text"
                @click.stop="imgFd($event)"
                v-html="content(item.content, item.id == user.id)"
              ></div>
            </div>
          </li>
        </ul>
    ...
    <MshowModal v-if="shareShow" @close="shareShow =false" @selectedMan="share" :group="true"></MshowModal>
  </div>
</template>
<style lang="less">
.chat-file {
